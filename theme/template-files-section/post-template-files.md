...
あなたのブログ投稿のほとんどは、同じ表示形式にしたいでしょうが、すべてのレシピには材料と手順のセクションがあるので、レシピ・カテゴリーに分類される投稿は例外です。したがって、`category-recipe.php` ファイルを作成し、あなたのレシピ・ブログ投稿をグリッド・ビューで表示するとともに、レシピに関する重要な詳細の一部を見えるようにすることをおすすめします。
