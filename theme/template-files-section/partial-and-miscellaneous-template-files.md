...
多くのテーマ開発者は、テンプレート・ファイルを小さな単位に分割します。彼らはよく、ラッパーやページ構造要素を `page.php`、`home.php`、`comments.php` などのテンプレート・ファイルに配置し、それらのページのコンテンツを実際に表示するコードは、別のテンプレート・ファイルに配置します。ここで登場するのが `content-{$slug}.php` です。一般的な例としては、`content-page.php`、`content-post.php`、`content-portfolio.php`、`content-none.php` などがあります。これらは WordPress が認識し特定の方法で解釈するファイル名ではなく、特定の種類のコンテンツを表示するための一般的なアプローチです。
